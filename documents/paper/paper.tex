...
\begin{itemize}
	...
	\item NVIDIA GeForce GTX 750 (512 CUDA cores, 1653 MHz core clock, 6510 MHz memory clock, 2 GB video memory)
\end{itemize}
...
\end{enumerate}
Table \ref{real-time-fps-statistics} shows basic statistics on the frame rate results achieved on a $640 \times 480$ continuous video stream, running on the same hardware as the benchmarks in the previous section. The frame rate was evaluated on a 10-second video stream in a well-lit environment with motion of a moderate number of edge pixels ($< 10\%$ of the total number of pixels in the frame) in front of the camera.
\begin{table}[H]
	\small
	\centering
...
\section{Conclusion}
TODO: Some discussion of the results is warranted
