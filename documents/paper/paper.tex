...
\title{GPU Acceleration of Machine Learning-Aided Facial Recognition and Edge-Based Motion Detection with NVIDIA CUDA}
...
While CUDA is an excelent language for maximizing performance, it could be infeasible due to inexperience with C, time constraints, or lack of an NVIDIA GPU. To address these constraints, we also implemented our algorithms in Python using OpenCV. Python was chosen because it is very easy for inexperienced programmers to use and OpenCV was chosen because it offers many common image processing algorithms in a variety of languages. Unfortunately using OpenCV limits us to the library's implementation, does not currently support the use of GPU parallelization in Python. To take advantage of the GPU and parallelization in Python we would have to use C wrappers, which defeats the purpose of our implementation in Python.
...
\section*{Source Code}
The full source code for this project, including CUDA, Python, and MATLAB implementations of all of our algorithms, is open sourced and available at \texttt{https://github.com/LINKIWI/cuda-computer-vision}.
